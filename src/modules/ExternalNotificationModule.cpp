    ...
    */
    ...
    if (moduleConfig.external_notification.enabled) {
        ...
        if (moduleConfig.external_notification.output_buzzer) {
            ...
            // FIXME: move the `config.device.buzzer_gpio = config.device.buzzer_gpio ? config.device.buzzer_gpio : PIN_BUZZER;` to here and do it for both active and PWM buzzers, but need to check if there's a reason the variant file's buzzer pin isn't checked if using a passive buzzer.
